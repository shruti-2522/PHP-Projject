...
<body class="sb-nav-fixed">
    ...
    <div class="col-sm-4 form-group"><h1><b>EDIT CROP</b></h1></div>

    <?php
if(isset($_POST["btnsave"]))
{
  extract($_POST);
   mysqli_query($db,"update tblcrop set plot_no='$plot_no',pname='$pname',date='$txtdate',stime='$txtstime',etime='$txtetime',duration='$txtduration',prunning_day='$txtpday',title='$txttitle',observation='$txtobs',status=0 where cid=".$_GET["id"]);
  
   echo "<script>window.location.href='crop.php';</script>";
    exit;
  }
  $q1=mysqli_query($db,"select * from tblcrop where cid=".$_GET["id"]);
    ...
    <input type="text" placeholder="Enter End Time.." id="time3" class="form-control" name="txtduration" value="<?php echo $r1['duration'];?>">
              ...
              <div class="col-sm-6 form-group">
                <label><font color="black"><b>Title:</b></font></label>
                 <input  placeholder="Enter Corrective Measures.." name="txttitle" class="form-control"  id="txttitle" value="<?php echo $r1['title'];?>">
              </div>
            </div>
         ...
         <textarea name="txtobs" class="form-control" rows="5" cols="15" ><?php echo $r1['observation']?> </textarea>
       </div></div>
<!--               <div class="row">
            <div class="col-sm-6 form-group">
