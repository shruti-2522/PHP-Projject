...
if(isset($_POST["btnsave"]))
{
  ...
  // mysqli_query($db,"insert into tblinhousecal(srno,machine_name,used_interval,time_interval,date_interval,title,remark,ses_id)values('$cno','$machines','$txtused','$txttime','$txtdate','$txttitle','$txtremark','$ses_id')");
   ...
   mysqli_query($db,"update tblinhousecal set srno='$cno',machine_name='$machines',used_interval='$txtused',time_interval='$txttime',date_interval='$txtdate',title='$txttitle',remark='$txtremark' where inid=".$_GET["id"]);
  echo "<script>window.location.href='inhouse_calibbration1.php';</script>";
    exit;
}
$q1=mysqli_query($db,"select * from tblinhousecal where inid=".$_GET["id"]);
  $r1=mysqli_fetch_array($q1);
?>
      ...
      ?>
       <input type="text" class="form-control"  id="cno" name="cno" value="<?php echo $r2['cnt']+1; ?>" required>
        ...
        <option value="<?php echo $r1['machine_name']; ?>"selected><?php echo $r1['machine_name']; ?></option>
              ...
              <?php
               $ses_id=$_SESSION['plot_id'];
                    $q=mysqli_query($db,"select distinct(tblntype.name_of_sprayer) as sname from tblsprayer,tblntype where tblntype.nid=tblsprayer.sprayer_name where ses_id=".$ses_id);
...
<div class="col-sm-1"></div>
       ...
       <label><font color="black"><b>Title Of Work:</b></font></label>
         <input type="text" class="form-control" name="txttitle" id="txttitle" placeholder="Enter Title" value="<?php echo $r1['title']?>">
   </div>
 </div>
...
<div class="row">
   ...
   <div class="col-sm-8">
             <button type="Submit" class="btn btn-success col-sm-3" name="btnsave"><b>UPDATE</b></button>
         </div>
         <br>
       </div>
</div>
