...
*/
  ...
  if(isset($_POST["btncancel"]))
  {
   ...
   $q=mysqli_query($db,"select * from tblfertsession where status=0 and ses_id=".$ses_id);
   while($r=mysqli_fetch_array($q))
   {
   $sum=0;
   $q1=mysqli_query($db,"select * from tblp1 where item_name='".$r['fert_name']."' and ses_id=".$ses_id);
   $r1=mysqli_fetch_array($q1); 
   
   $sum=$r1['reduce_qty']+$r['Aqty_app'];
  
     mysqli_query($db,"update tblp1 set reduce_qty='".$sum."' where item_name='".$r['fert_name']."' and ses_id=".$ses_id); 
  
     mysqli_query($db,"DELETE FROM tblfertsession WHERE fid=".$r['fid']);

     mysqli_query($db,"DELETE FROM tblfert1 WHERE fert_id=".$r['fert_id']);
...
<button type="submit" name="btncancel" id="btncancel" class="btn btn-danger" formnovalidate><b>CANCEL</b></button>
